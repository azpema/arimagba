...
class Utils {
  public:
    ...
    static bool getRegSingleBit(uint64_t reg, uint8_t pos);
    ...
    static std::string toHexString(uint32_t val, int paddingNum = 0);

    static constexpr uint32_t byteSwap32(uint32_t val);
    
    static uint32_t readUint32(uint8_t* data);
    static uint64_t readUint64(uint8_t* data);
