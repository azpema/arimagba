...
Most internal memory is mirrored across the whole 24bit/16MB address space in which it is located:
  ...
  VRAM at 6XXXXXXh,
  OAM at 7XXXXXXh.
Even though VRAM is sized 96K (64K+32K), it is repeated in steps of 128K (64K+32K+32K, the two 32K blocks itself being
mirrors of each other).

BIOS ROM, Normal ROM Cartridges, and I/O area are NOT mirrored, the only exception is the undocumented I/O port at
4000800h (repeated each 64K).

The 64K SRAM area is mirrored across the whole 32MB area at E000000h-FFFFFFFh, also, inside of the 64K SRAM field, 32K
SRAM chips are repeated twice.

*/

class MemoryManager {
  public:
    MemoryManager(BIOS& bios,
                  GamePak& gamepak,
                  VRAM& vram,
                  EWRAM& ewram,
                  IWRAM& iwram,
