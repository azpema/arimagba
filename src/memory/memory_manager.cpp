...
uint16_t MemoryManager::readHalfWord(uint32_t addr, bool opPreFetch) { return read(addr, 2, opPreFetch); }

uint8_t MemoryManager::readByte(uint32_t addr) { return read(addr, 1); }
...
uint32_t MemoryManager::read(uint32_t addr, uint8_t bytes, bool opPreFetch) {
    ...
    case Region::VRAM:
        if (Utils::inRange<>(addr, VRAM_OFFSET_START, VRAM_OFFSET_END)) {
            ...
            // In bitmap modes reads and writes to 0x06018000 - 0x0601BFFF do not work (writes are discarded; reads may
            // always return 0?). https://github.com/nba-emu/hw-test/tree/master/ppu/vram-mirror
...
void MemoryManager::store(uint32_t addr, uint32_t val, uint8_t bytes) {
    ...
    case Region::VRAM: {
        ...
        // Video Memory (BG, OBJ, OAM, Palette) can be written to in 16bit and 32bit units only. Attempts to write 8bit
        // data (by STRB opcode) won’t work: Writes to OBJ (6010000h-6017FFFh) (or 6014000h-6017FFFh in Bitmap mode) and
        // to OAM (7000000h-70003FFh) are ignored, the memory content remains unchanged. Writes to BG
        // (6000000h-600FFFFh) (or 6000000h-6013FFFh in Bitmap mode) and to Palette (5000000h-50003FFh) are writing the
        // new 8bit value to BOTH upper and lower 8bits of the addressed halfword, ie. “[addr AND NOT 1]=data*101h”.
...
uint8_t* MemoryManager::getRawVRAM() { return vram.getRawMemory(); }

uint8_t* MemoryManager::getPaletteRAM() { return paletteRam.getRawMemory(); }

uint8_t* MemoryManager::getIOregisters() { return io.getRawMemory(); }

uint8_t* MemoryManager::getOAM() { return oam.getRawMemory(); }

void MemoryManager::addCpu(ARM7TDMI* cpu) { this->cpu = cpu; }
