...
IOregisters::IOregisters() : mustHaltCpu(false) {
    ...
    for (size_t n = 0; n < DMA_CNT; n++) {
        ...
        DMAxDAD[n] =
            reinterpret_cast<uint32_t*>(mem.get() + (REG_ADDR::DMAxDAD[n] - MemoryManager::IO_REGISTERS_OFFSET_START));
        DMAxCNT_L[n] = reinterpret_cast<uint16_t*>(mem.get() +
                                                   (REG_ADDR::DMAxCNT_L[n] - MemoryManager::IO_REGISTERS_OFFSET_START));
        DMAxCNT_H[n] = reinterpret_cast<uint16_t*>(mem.get() +
                                                   (REG_ADDR::DMAxCNT_H[n] - MemoryManager::IO_REGISTERS_OFFSET_START));
    }

    for (size_t n = 0; n < BG_CNT; n++) {
        BGxCNT[n] =
            reinterpret_cast<uint16_t*>(mem.get() + (REG_ADDR::BGxCNT[n] - MemoryManager::IO_REGISTERS_OFFSET_START));
    }
...
uint8_t IOregisters::getDCNT_MODE() { return Utils::getRegBits(getDISPCNT(), 0b0000000000000111, 0); }

bool IOregisters::getMustHaltCpu() { return mustHaltCpu; }

void IOregisters::setMustHaltCpu() { mustHaltCpu = true; }

void IOregisters::clearMustHaltCpu() { mustHaltCpu = false; }
