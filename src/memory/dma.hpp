...
template <int N>
DMA<N>::DMA(ARM7TDMI& cpu, MemoryManager& mem) :
    cpu(cpu), mem(mem), dmaNum(N), io(mem.getIOregisters()),
...
template <int N> void DMA<N>::runCycle(const bool vblankNow, const bool hblankNow) const {
    ...
    if (isDmaEnabled()) {
            ...
            */
            if (timingMode == TimingMode::DMA_AT_REFRESH) {
                if (dmaNum == 1 || dmaNum == 2) {
                    ...
                    throw std::runtime_error("ERROR: Unimplemented DMA_AT_REFRESH timing from DMA " +
                                             std::to_string(dmaNum));
                }
            }
        }
        ...
        // std::cout << "src: " << Utils::toHexString(readAddr) << " dst: " << Utils::toHexString(writeAddr) << " " <<
        // Utils::toHexString(*DMAxCNT_H) << ":" << Utils::toHexString(*DMAxCNT_L) << "\n";
        ...
        if (doTransferNow) {
            ...
            if (isRepeatEnabled() &&
                (timingMode == TimingMode::DMA_AT_VBLANK || timingMode == TimingMode::DMA_AT_VBLANK)) {
...
template <int N> typename DMA<N>::ChunkSize DMA<N>::getChunkSize() const {
...
template <int N> typename DMA<N>::TimingMode DMA<N>::getTimingMode() const {
...
template <int N> bool DMA<N>::isIrqEnabled() const {
...
*/
template <int N> uint32_t DMA<N>::getNumTransfers() const {
    if (*DMAxCNT_L == 0) {
        if (N == 3) {
            return 0x10000;
        } else {
...
template <int N> uint8_t DMA<N>::getTransferSizeBytes() const {
    const static uint8_t transferSizeBytes[2] = {2, 4};
    return transferSizeBytes[static_cast<uint8_t>(getChunkSize())];
}

template <int N> void DMA<N>::disableDma() const { *DMAxCNT_H &= ~REG_DMAxCNT::DMA_ENABLE_MASK; }

template <int N> bool DMA<N>::isSourceAddrValid(int dma, uint32_t sourceAddr) {
    if (dma == 0 && sourceAddr >= MemoryManager::GAMEPAK_WAIT_0_OFFSET_START &&
        sourceAddr < MemoryManager::GAMEPAK_SRAM_OFFSET_START) {
        return false;
    }
    return sourceAddr >= MemoryManager::EWRAM_OFFSET_START;
}

template <int N> bool DMA<N>::isDestAddrValid(int dma, uint32_t address) {
