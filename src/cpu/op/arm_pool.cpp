...
ArmPool::ArmPool(ARM7TDMI& cpu) {
    ...
    armPool.insert(
        {ArmOpcode::OpCodeEnum::HALFWORD_DATA_TRANSFER_REGISTER, std::make_unique<HalfwordDataTransferRegister>(cpu)});
    armPool.insert(
        {ArmOpcode::OpCodeEnum::HALFWORD_DATA_TRANSFER_OFFSET, std::make_unique<HalfwordDataTransferOffset>(cpu)});
...
ArmOpcode* ArmPool::getArmInstance(ArmOpcode::OpCodeEnum key) const { return armPool.at(key).get(); }