...
MultipleLoadStore::MultipleLoadStore(uint16_t op, ARM7TDMI& cpu) : ThumbOpCode::ThumbOpCode(op, cpu) { init(op); }
...
std::string MultipleLoadStore::getOpMnemonic() { return op2Mnemonic[l]; }
...
void MultipleLoadStore::doExecute() {
    ...
    BlockDataTransfer* opArm =
        static_cast<BlockDataTransfer*>(cpu.getArmPool().getArmInstance(ArmOpcode::OpCodeEnum::BLOCK_DATA_TRANSFER));
    opArm->init(p, u, 0, 1, l, rb, rList);
    opArm->doExecute();
    // std::cout << "<< ARM >> " << opArm.toString() << '\n';
}

uint32_t MultipleLoadStore::cyclesUsed() const { return 1; }