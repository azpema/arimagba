...
AddOffsetSP::AddOffsetSP(uint16_t op, ARM7TDMI& cpu) : ThumbOpCode::ThumbOpCode(op, cpu) { init(op); }
...
std::string AddOffsetSP::getSMnemonic() { return s2Mnemonic[s]; }

std::string AddOffsetSP::toString() { return "add sp,#" + getSMnemonic() + Utils::toHexString(offsetStr); }
...
uint32_t AddOffsetSP::cyclesUsed() const { return 1; }