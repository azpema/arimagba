...
LoadStoreHalfword::LoadStoreHalfword(uint16_t op, ARM7TDMI& cpu) : ThumbOpCode::ThumbOpCode(op, cpu) { init(op); }
...
std::string LoadStoreHalfword::getOpMnemonic() { return op2Mnemonic[l]; }
...
void LoadStoreHalfword::doExecute() {
    HalfwordDataTransferOffset opArm =
        HalfwordDataTransferOffset(1, 1, 0, l, rb, rd, 0, 1, (offsetVal & 0xF0) >> 4, offsetVal & 0xF, cpu);
    opArm.doExecute();
    // std::cout << "<< ARM >> " << opArm.toString() << '\n';
}

uint32_t LoadStoreHalfword::cyclesUsed() const { return 1; }