...
ShiftRm::ShiftRm(uint16_t val) :
    Operand::Operand(val, OperandType::SHIFT_RM), c(false), _shift(Utils::getRegBits(val, SHIFT_MASK, SHIFT_SHIFT)),
    _rm(Utils::getRegBits(val, RM_MASK, RM_SHIFT)),
    _shiftType(Utils::getRegBits(_shift, SHIFT_TYPE_MASK, SHIFT_TYPE_SHIFT)), shiftAmount(0xDEAD) {
    // Shift amount
...
ShiftRm::ShiftRm(uint8_t rm, bool typeShiftAmount, uint8_t amount, uint8_t type, uint8_t rs) :
    Operand::Operand(0xDEAD, OperandType::SHIFT_RM), c(false), _rm(rm), _shiftType(type), shiftAmount(0xDEAD) {
    // Shift amount
    if (typeShiftAmount) {
        shiftAmount = amount;
        _shiftAmount = amount;
...
std::string ShiftRm::getShiftTypeMnemonic() { return shiftType2Mnemonic[_shiftType]; }

uint16_t ShiftRm::getShiftType() { return _shiftType; }

uint16_t ShiftRm::getRm() { return _rm; }

uint16_t ShiftRm::getShiftAmount() { return shiftAmount; }

ShiftRm::Type ShiftRm::getType() { return type; }

uint16_t ShiftRm::getShiftReg() { return _shiftReg; }

bool ShiftRm::getC() { return c; }
...
uint32_t ShiftRm::getOperandVal(ARM7TDMI& cpu) {
    ...
    if (type == AMOUNT && shiftAmount == 0) {
        ...
        case ROR:
            ...
            break;
        default:
            throw std::runtime_error("ERROR: Invalid _shiftType value! type: " + std::to_string(type) +
                                     " shiftAmount: " + std::to_string(shiftAmount));
            break;
        }
    }
    if (type == REGISTER) {
