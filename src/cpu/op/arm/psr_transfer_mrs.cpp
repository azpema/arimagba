...
PSRTransferMRS::PSRTransferMRS(uint32_t op, ARM7TDMI& cpu) : ArmOpcode::ArmOpcode(op, cpu) { init(op); }
...
std::string PSRTransferMRS::getPSRMnemonic() { return PSR2Mnemonic[psr]; }
...
// MSR,MRS          1S
uint32_t PSRTransferMRS::cyclesUsed() const { return 1 * ARM7TDMI::CPU_CYCLES_PER_S_CYCLE; }