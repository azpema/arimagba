...
std::string ArmOpcode::toHexString(){
...
bool ArmOpcode::isBranch(uint32_t op){
    return OpCode::checkOpCode(op, BRANCH_MASK, BRANCH_FORMAT);
}
...
bool ArmOpcode::isUndefined(uint32_t op){
    return OpCode::checkOpCode(op, UNDEFINED_MASK, UNDEFINED_FORMAT);
}
...
bool ArmOpcode::isMultiply(uint32_t op){
    return OpCode::checkOpCode(op, MULTIPLY_MASK, MULTIPLY_FORMAT);
}
...
ArmOpcode::Condition ArmOpcode::getCondField(){
    return cond;
}

std::string ArmOpcode::getCondFieldMnemonic(){
