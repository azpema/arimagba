...
SingleDataSwap::SingleDataSwap(uint32_t op, ARM7TDMI& cpu) : ArmOpcode::ArmOpcode(op, cpu) { init(op); }

SingleDataSwap::SingleDataSwap(ARM7TDMI& cpu) : ArmOpcode::ArmOpcode(cpu) {}

void SingleDataSwap::init(uint32_t op) {
    ArmOpcode::init(op);
...
std::string SingleDataSwap::getBFlagMnemonic() { return bFlag2Mnemonic[b]; }
...
uint32_t SingleDataSwap::cyclesUsed() const {
    return 1 * ARM7TDMI::CPU_CYCLES_PER_S_CYCLE + 2 * ARM7TDMI::CPU_CYCLES_PER_N_CYCLE +
           1 * ARM7TDMI::CPU_CYCLES_PER_I_CYCLE;
}