...
ThumbPool::ThumbPool(ARM7TDMI& cpu) {
    ...
    thumbPool.insert(
        {ThumbOpCode::OpCodeEnum::HI_REGISTER_BRANCH_EXCHANGE, std::make_unique<HiRegisterBranchExchange>(cpu)});
    thumbPool.insert({ThumbOpCode::OpCodeEnum::PC_RELATIVE_LOAD, std::make_unique<PCRelativeLoad>(cpu)});
    thumbPool.insert(
        {ThumbOpCode::OpCodeEnum::LOAD_STORE_REGISTER_OFFSET, std::make_unique<LoadStoreRegisterOffset>(cpu)});
    thumbPool.insert({ThumbOpCode::OpCodeEnum::LOAD_STORE_SIGN_EXTENDED, std::make_unique<LoadStoreSignExtended>(cpu)});
    thumbPool.insert(
        {ThumbOpCode::OpCodeEnum::UNCONDITIONAL_BRANCH, std::make_unique<Thumb::UnconditionalBranch>(cpu)});
    ...
    thumbPool.insert(
        {ThumbOpCode::OpCodeEnum::LONG_BRANCH_WITH_LINK, std::make_unique<Thumb::LongBranchWithLink>(cpu)});
...
ThumbOpCode* ThumbPool::getThumbInstance(ThumbOpCode::OpCodeEnum key) const { return thumbPool.at(key).get(); }