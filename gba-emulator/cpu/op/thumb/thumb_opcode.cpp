...
bool ThumbOpCode::isAddSubtract(uint16_t op){
    return OpCode::checkOpCode(op, ADD_SUBTRACT_MASK, ADD_SUBTRACT_FORMAT);
}
...
bool ThumbOpCode::isLoadAddress(uint16_t op){
    return OpCode::checkOpCode(op, LOAD_ADDRESS_MASK, LOAD_ADDRESS_FORMAT);
}
